package com.asto.dmp.alipay.dao

import com.asto.dmp.alipay.base.{Constants, Contexts}
...
trait Dao extends Logging {
  ...
  private def combineSqlAndQuery(tempTableName: String, sqlObj: SQL): RDD[Array[String]] = {
    ...
    val resultRDD = sqlContext.sql(_sql).map(a => (for(i <- 0 until a.length) yield a(i).toString).toArray)
